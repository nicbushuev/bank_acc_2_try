...
def test_account_number_parametrize(account_number, expected):
	"""Тест с маскировки различных номеров счета с параметризацией"""
	assert get_mask_account(account_number) == expected


@pytest.mark.parametrize("invalid_account_number, error_match", [
	("", "Отсутствует номер счета"),
	(" ", "В номере счета не должно быть пробелов"),
	("123", "Неверный формат ввода! Должно быть 20 цифр"),
	("1234abcd123412341234", "Неверный формат ввода, не должно быть букв в номере счета"),
])
def test_get_mask_account_invalid(invalid_account_number, error_match):
	"""Тест на отработку ошибок ValueError с параметризацией"""
	with pytest.raises(ValueError, match=error_match):
